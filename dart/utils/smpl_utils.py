...
_body_model_dict_cache = {}

def _get_body_model_dict():
    if not _body_model_dict_cache:
        try:
            _body_model_dict_cache['male'] = smplx.build_layer(body_model_dir, model_type='smplx',
                                  gender='male', ext='npz',
                                  num_pca_comps=12)
            _body_model_dict_cache['female'] = smplx.build_layer(body_model_dir, model_type='smplx',
                                    gender='female', ext='npz',
                                    num_pca_comps=12)
        except Exception as e:
            print(f'[WARNING] Failed to load SMPL-X body model: {e}')
            print('[WARNING] Body model operations (export_smpl, joint regression) will not work.')
            return None
    return _body_model_dict_cache

class _LazyBodyModelDict:
    def __getitem__(self, key):
        d = _get_body_model_dict()
        if d is None:
            raise RuntimeError('SMPL-X body model not loaded. Please download SMPL-X model files from https://smpl-x.is.tue.mpg.de/ and place them in: ' + str(body_model_dir))
        return d[key]
...
body_model_dict = _LazyBodyModelDict()
...
class PrimitiveUtility:
    def __init__(self, device='cpu', dtype=torch.float32, motion_repr=None, body_type='smplx'):
        ...
        self._bm_male = None
        self._bm_female = None
        self._bm_loaded = False

    def _load_body_models(self):
        if self._bm_loaded:
            return
        if self.body_type == 'smplx':
            self._bm_male = body_model_dict['male'].to(self.device).eval()
            self._bm_female = body_model_dict['female'].to(self.device).eval()
        else:
            ...
            self._bm_male = smplh_body_model_dict['male'].to(self.device).eval()
            self._bm_female = smplh_body_model_dict['female'].to(self.device).eval()
        self._bm_loaded = True

    @property
    def bm_male(self):
        self._load_body_models()
        return self._bm_male

    @property
    def bm_female(self):
        self._load_body_models()
        return self._bm_female

    def get_smpl_model(self, gender):
        self._load_body_models()
        return self._bm_male if gender == 'male' else self._bm_female
    ...
    def get_new_coordinate(self, body_param_dict, use_predicted_joints=False, pred_joints=None):
        if use_predicted_joints:
            ...
            if not self._bm_loaded:
                try:
                    self._load_body_models()
                except:
                    pass
            if not self._bm_loaded:
                # Fallback: create identity rotation and use transl as new coordinate
                B = body_param_dict['transl'].shape[0]
                device = body_param_dict['transl'].device
                dtype = body_param_dict['transl'].dtype
                new_rotmat = torch.eye(3, device=device, dtype=dtype).unsqueeze(0).repeat(B, 1, 1)
                transl = body_param_dict['transl']
                new_transl = transl.unsqueeze(1) if transl.dim() == 2 else transl[:, :1, :]
                return new_rotmat, new_transl
    ...
    def calc_calibrate_offset(self, body_param_dict):
        if not self._bm_loaded:
            try:
                ...
                pass
        if not self._bm_loaded:
            # Fallback: return zero offset when body model is unavailable
            B = body_param_dict['betas'].shape[0]
            return torch.zeros(B, 3, device=body_param_dict['betas'].device, dtype=body_param_dict['betas'].dtype)
        body_model = self.bm_male if body_param_dict['gender'] == 'male' else self.bm_female
    ...
    def calc_features(self, primitive_dict, use_predicted_joints=False):
        ...
        B, T, _ = primitive_dict['transl'].shape
        if use_predicted_joints or 'joints' in primitive_dict:
            ...
            if not self._bm_loaded:
                try:
                    ...
                    pass
            if not self._bm_loaded:
                # Fallback: create dummy joints from transl (pelvis only, 22 joints at pelvis)
                transl = primitive_dict['transl'].reshape(B * T, 3)
                output_joints = transl.unsqueeze(1).repeat(1, 22, 1).reshape(B, T, 22, 3)
            else:
